...
public class BlockLabelUI implements UI {
    ...
    public void render(Graphics2D g2d, BaseUI component) {
        ...
        if (blockLabel.isShowAllText()) {
            int delta = 0;
            if (blockLabel.getAlign().x >= 0) {
                delta = blockLabel.getWidth() - fm.stringWidth(text);
                if (blockLabel.getAlign().x == 0) delta /= 2;
            }
            g2d.drawString(text, blockLabel.getAbsoluteX() + delta + blockLabel.getOffsetX(), blockLabel.getAbsoluteY() + fm.getMaxAscent() - fm.getDescent() / 2 + blockLabel.getOffsetY());
        } else {
            String repeatText = "...";
            ...
            if (fm.stringWidth(text) > blockLabel.getWidth()) {
                if (repeatWidth <= blockLabel.getWidth()) {
                    ...
                    g2d.drawString(sb.toString(), blockLabel.getAbsoluteX() + delta + blockLabel.getOffsetX(), blockLabel.getAbsoluteY() + fm.getMaxAscent() - fm.getDescent() / 2 + blockLabel.getOffsetY());
                }
            } else {
                int delta = 0;
                if (blockLabel.getAlign().x >= 0) {
